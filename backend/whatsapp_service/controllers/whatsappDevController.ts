...
class WhatsappDevController {
  private updateUserConversation = async (userId: any, localChats: any) => {
    try {
  ...
  private checkForExcessiveMessages = (chats: any) => {
    try {
  ...
  handleWhatsappMessage = async (message: any) => {
    try {
      ...
      if (isUserAuthorized) {
        ...
        if (chats) {
          ...
          const isSpamming = this.checkForExcessiveMessages(chats);
          if (isSpamming) {
        ...
        const updatedChats = await this.updateUserConversation(
          user._id,
          localChats
        );
...
export default new WhatsappDevController();
